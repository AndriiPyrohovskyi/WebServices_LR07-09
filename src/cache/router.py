...
async def set_cache_value(data: CacheSetRequest, redis: Redis = Depends(get_redis)) -> CacheSetResponse:
    """
    ...
    try:
        ...
        return CacheSetResponse(success=True, key=data.key, message=message, ttl=data.ttl)

    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to set cache value: {str(e)}"
        )


@router.get("/get/{key}", response_model=CacheGetResponse)
async def get_cache_value(key: str, redis: Redis = Depends(get_redis)) -> CacheGetResponse:
    """
    ...
    try:
        ...
        return CacheGetResponse(key=key, value=value, exists=value is not None)

    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get cache value: {str(e)}"
        )


@router.delete("/delete/{key}")
async def delete_cache_value(key: str, redis: Redis = Depends(get_redis)):
    """
    ...
    try:
        ...
        return {"success": True, "key": key, "message": "Key deleted successfully"}

    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to delete cache value: {str(e)}"
        )


@router.get("/keys")
async def list_cache_keys(pattern: str = "*", redis: Redis = Depends(get_redis)):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to list cache keys: {str(e)}"
        )
